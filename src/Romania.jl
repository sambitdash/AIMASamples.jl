...
import ..core:result,step_cost,goal_test,search,actions
...
type RomaniaRoadMapProblem{SA<:SearchAlgorithm} <: Problem
...
function goal_test(problem::RomaniaRoadMapProblem, state::In)
...
function in(state::In,
            nodes::PriorityQueue{In,Node{In},Base.Order.ForwardOrdering})
  return haskey(nodes,state)
end
...
const solveRomanianMapProblemResultMinCost=
  [(:Arad, 0),(:Sibiu, 140),(:RimnicuVilcea,220),
   (:Pitesti,317),(:Bucharest,418)]
